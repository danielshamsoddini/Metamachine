...
class FrozenJointsProcessor:
    ...
    def __init__(self, cfg: OmegaConf, total_joints: int) -> None:
        ...
        self.active_joints = [
            i for i in range(total_joints) if i not in self.frozen_indices
        ]
    ...
    def _setup_frozen_joints(self, cfg) -> None:
        """Parse and validate frozen joints configuration."""
        frozen_config = cfg.environment.get("frozen_joints", {})

        if frozen_config is None or (
            isinstance(frozen_config, (list, tuple, ListConfig))
            and len(frozen_config) == 0
        ):
            # No frozen joints
            self.frozen_indices = []
        ...
        if (
            isinstance(frozen_config, (list, tuple, ListConfig))
            and len(frozen_config) > 0
            and all(isinstance(x, int) for x in frozen_config)
        ):
            self.frozen_indices = list(frozen_config)
            self.frozen_patterns = {
                idx: {"type": "zero"} for idx in self.frozen_indices
            }
            ...
            for joint_spec in frozen_config:
                if isinstance(joint_spec, int):
                    ...
                    self.frozen_indices.append(idx)
                    self.frozen_patterns[idx] = dict(
                        joint_spec
                    )  # Convert to regular dict
                    del self.frozen_patterns[idx][
                        "joint"
                    ]  # Remove joint index from pattern
        else:
        ...
        if self.frozen_indices and max(self.frozen_indices) >= self.total_joints:
            raise ValueError(
                f"Frozen joint index {max(self.frozen_indices)} >= total joints {self.total_joints}"
            )

        if len(set(self.frozen_indices)) != len(self.frozen_indices):
            raise ValueError("Duplicate frozen joint indices found")

    def _setup_pattern_generators(self) -> None:
        ...
        for idx in self.frozen_indices:
            ...
            if pattern_type == "random":
                ...
                self.pattern_state[idx] = {"last_value": 0.0, "next_change_time": 0.0}
            elif pattern_type == "sine":
                # Initialize sine wave state
                self.pattern_state[idx] = {"phase": pattern.get("phase", 0.0)}
    ...
    def map_action_to_full_space(self, reduced_action: np.ndarray) -> np.ndarray:
        ...
        if reduced_action.ndim == 1:
            ...
            full_action = np.zeros(
                (batch_size, self.total_joints), dtype=reduced_action.dtype
            )
            full_action[:, self.active_joints] = reduced_action
...
class ActionProcessor:
    ...
    def __init__(self, cfg: OmegaConf) -> None:
        ...
        self.last_action_full = np.zeros(
    ...
    def _setup_position_reference(self, cfg: OmegaConf) -> None:
        ...
        self.position_reference_repeat_steps = int(reference_cfg.get("repeat_steps", 0))
    ...
    def _reference_target_for_step(self, step: int) -> np.ndarray:
        if self.position_reference_repeat_steps > 0:
            step = int(step) % self.position_reference_repeat_steps
    ...
    def _setup_filters(self, cfg) -> None:
        ...
        if self.use_melter:
            self.melter = ActionFilterMelter(axis=cfg.control.melter.axis)
        else:
            self.melter = None
    ...
    def get_action_bounds_for_active_joints(self):
        ...
        if self.action_bounds.custom_limits is not None:
            ...
            active_limits = self.action_bounds.custom_limits[
                self.frozen_processor.active_joints
            ]
            return ActionBounds(
                limit=self.action_bounds.symmetric_limit, custom_limits=active_limits
            )
        else:
...
class ActionBounds:
    ...
    def __init__(
        self,
        limit: float,
        custom_limits: Optional[Union[list[float], np.ndarray]] = None,
    ):
...
def run_example():
    ...
    for step in range(10):
        ...
        print(
            f"{step:4d} | {processed[1]:11.3f} | {processed[3]:12.3f} | {processed[5]:12.3f}"
        )
    print()
    ...
    for step in range(15):
        ...
        print(
            f"{step:4d} | {processed[0]:4.1f} | {processed[1]:5.1f} | {processed[2]:5.2f} | {processed[3]:6.2f} | {processed[4]:5.2f} | {processed[5]:6.2f}"
        )
