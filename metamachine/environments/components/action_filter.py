...
class ActionFilter:
    ...
    def __init__(self, a, b, order, num_joints, ftype="lowpass") -> None:
        """Initializes filter.
        ...
        if ftype == "bandpass":
            ...
            raise ValueError(f"{ftype} filter type not supported")
    ...
    def reset(self, init_hist=None) -> None:
...
class ActionFilterButter(ActionFilter):
    ...
    def __init__(
        ...
        a_coeffs = []
        b_coeffs = []
        for i, (low_freq, high_freq) in enumerate(zip(self.lowcut, self.highcut)):
            if high_freq <= 0.0:
                raise ValueError("Highcut must be > 0")

            b, a = self.butter_filter(low_freq, high_freq, sampling_rate, order)
            logging.info(
...
#         if np.std(self.x_hist, axis=0)[idx] < 0.2 :
#             print("MELTING")
#             # return np.ones_like(x)
#             # return np.random.normal(np.zeros_like(x), 2)
#             x[idx] = np.random.normal(0, 2)
#             # import pdb; pdb.set_trace()

#         else:
#             print(np.std(self.x_hist, axis=0)[idx])
...
class ActionFilterMelter:
    ...
    def filter(self, x):
        self.x_hist.append(x[self.idx])
        if len(self.x_hist) > 20:
            self.x_hist.pop(0)

        if np.std(self.x_hist) < 0.1:
            print("MELTING")
            x[self.idx] = np.random.normal(0, 2)

        else:
            print(f"STD: {np.std(self.x_hist)}")
