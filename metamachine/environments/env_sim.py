...
class MetaMachine(Base, MujocoEnv):
    ...
    def start_video_recording(self) -> None:
        ...
        if self.render_mode == "mp4":
            ...
            self.trajectory_command_segments = []
        ...
        if self.render_mode == "mp4" and self.recording_active:
            ...
            if self.sim_cfg.get("plot_trajectory", False) and hasattr(self, "trajectory_xs") and len(self.trajectory_xs) > 0:
                try:
                    ...
                    command_segments = getattr(self, "trajectory_command_segments", [])
                    if command_segments:
                        span = max(
                        ...
                        for index, (start_x, start_y, command_direction) in enumerate(command_segments):
                            plt.arrow(
                                start_x, start_y,
                                arrow_length * command_direction[0],
                                arrow_length * command_direction[1],
                                color=colors[index % len(colors)], width=0.01 * arrow_length,
