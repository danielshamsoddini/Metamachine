...
class TerrainBuilder:
    def __init__(self, terrain_type: TerrainType, worldbody: Any) -> None:
        if terrain_type is None:
            terrain_type = TerrainType.FLAT  # Default to flat terrain if not specified
        self.terrain_type = terrain_type
        self.worldbody = worldbody
        self.obstacle_idx_counter = 0  # Initialize counter

    def build(self) -> None:
        if self.terrain_type == TerrainType.FLAT:
            self._add_flat()
        elif self.terrain_type == TerrainType.STAIRS:
            self.add_stairs()  # Use the public method
        elif self.terrain_type == TerrainType.WALLS:
            self.add_walls()  # Use the public method
        else:
            raise NotImplementedError(
                f"Terrain type {self.terrain_type} is not supported yet."
            )
    ...
    def set_terrain(self, terrains, terrain_params=None) -> None:
        ...
        if "bumpy" in terrains:
            self.add_bumps(
                h_func=lambda x: 0.04 + 0.02 * x if 0.04 + 0.02 * x < 0.1 else 0.1
            )
        if "walls" in terrains:
            transparent = (
                terrain_params["transparent"]
                if terrain_params is not None and "transparent" in terrain_params
                else True
            )
            angle = (
                terrain_params["angle"]
                if terrain_params is not None and "angle" in terrain_params
                else False
            )
        ...
        if "random_bumpy" in terrains:
            ...
            for _ in range(terrain_params["num_bumps"]):
                ...
                self.add_minibump(
                    pos=pos, angle=angle, height=height, width=width, length=1
                )
        if "stairs" in terrains:
            num_steps = (
                20
                if terrain_params is None or "num_steps" not in terrain_params
                else terrain_params["num_steps"]
            )
            step_height = (
                0.1
                if terrain_params is None or "step_height" not in terrain_params
                else terrain_params["step_height"]
            )
            width = (
                0.5
                if terrain_params is None or "width" not in terrain_params
                else terrain_params["width"]
            )
            reserve_area = (
                2
                if terrain_params is None or "reserve_area" not in terrain_params
                else terrain_params["reserve_area"]
            )
            for i in range(reserve_area, num_steps + reserve_area):
                ...
                for p in pos:
                    ...
                    self.add_minibump(
                        pos=p, angle=angle, height=height, width=width, length=length
                    )
        ...
        if "dune" in terrains:
            radius_x = (
                20
                if terrain_params is None or "radius_x" not in terrain_params
                else terrain_params["radius_x"]
            )
            radius_y = (
                20
                if terrain_params is None or "radius_y" not in terrain_params
                else terrain_params["radius_y"]
            )
            elevation_z = (
                0.5
                if terrain_params is None or "elevation_z" not in terrain_params
                else terrain_params["elevation_z"]
            )
            base_z = (
                0.01
                if terrain_params is None or "base_z" not in terrain_params
                else terrain_params["base_z"]
            )
            self.set_hfield(
                file="dune.png",
                radius_x=radius_x,
    ...
    def add_walls(self, transparent: bool = False, angle: float = 0) -> None:
        ...
        wall_attrs = {"type": "box", "material": "boundary", "size": "25 0.1 0.5"}
        if transparent:
            wall_attrs["rgba"] = "0.1 0.1 0.1 0.0"

        XMLBuilder.create_element(
            wall, "geom", name="boundary/right", pos="0 1 0.25", **wall_attrs
        )
        XMLBuilder.create_element(
            wall, "geom", name="boundary/left", pos="0 -1 0.25", **wall_attrs
        )
        ...
        for i in range(num_steps):
            if direction == "x":
                ...
                self._create_box_xml(
                    f"step{i}", x, y, z, step_depth, step_width, step_height
                )
            else:  # direction == 'y'
                x, y, z = 0, i * step_depth + start_distance, i * step_height
                self._create_box_xml(
                    f"step{i}", x, y, z, step_width, step_depth, step_height
                )
    ...
    def set_hfield(
        ...
        """
        hfield_path = os.path.join(
            METAMACHINE_ROOT_DIR, "sim", "assets", "hfields", file
        )
