...
from ... import METAMACHINE_ROOT_DIR, get_asset_path
...
class RobotConfig:
    ...
    Attributes:
        ...
        length (float): Length parameter in meters (default: 0.3)
    ...
    def _validate_config(self) -> None:
        ...
        if not 0 <= self.theta <= np.pi:
            raise ConfigurationError(
                f"Theta must be between 0 and pi, got {self.theta}"
            )
        if self.length <= 0:
...
class Port:
    ...
    pos: Vector3
    body_pos: Vector3 | None = None
    leg_node: etree.Element | None = None
    leg_vec: Vector3 | None = None
    module_idx: int = 0
...
class MeshManager:
    ...
    @classmethod
    def validate_mesh_dict(cls, mesh_dict: dict[str, str] | None) -> None:
    ...
    def _is_valid_mesh_spec(cls, path: str) -> bool:
        ...
        if path.endswith(tuple(cls.SUPPORTED_MESH_TYPES)):
            try:
                mesh_path = get_asset_path("parts", path)
                if not os.path.exists(mesh_path):
                    raise MeshError(f"Mesh file not found: {mesh_path}")
            except FileNotFoundError as e:
                raise MeshError(f"Mesh file not found: {path} - {str(e)}") from e
        return True
...
class RobotBuilder:
    ...
    def __init__(
        self,
        mesh_dict: dict[str, str] | None = None,
        robot_cfg: dict | RobotConfig | None = None,
        ...
        try:
            ...
            raise RobotBuilderError(f"Failed to initialize robot: {str(e)}") from e
    ...
    def _add_size_settings(self) -> None:
        ...
        """
        XMLBuilder.create_element(self.root, "size", njmax="500", nconmax="100")
    ...
    def _init_robot_parameters(self) -> None:
        ...
        self.lleg_vec = np.array(
            [
                0,
                self.robot_cfg.length * np.cos(self.robot_cfg.theta),
                self.robot_cfg.length * np.sin(self.robot_cfg.theta),
            ]
        )
    ...
    def _get_mesh_file(self, mesh_name: str) -> str:
        ...
        try:
            file = get_asset_path("parts", mesh_name)
            if not os.path.exists(file):
                raise MeshError(f"Mesh {mesh_name} not found at {file}")
            return file
        except FileNotFoundError as e:
            raise MeshError(f"Mesh {mesh_name} not found: {str(e)}") from e

    def _convert_quat(self, quat: list[float] | list[list[float]]) -> np.ndarray:
        """
    ...
    def _add_one_module(
        ...
        l0 = self._create_module_body(
            parent, f"l{idx}", pos, quat, color_l, joint_params
        )
        ...
        r0 = self._create_module_body(
            parent, f"r{idx}", pos, quat, color_r, joint_params
        )
        ...
        XMLBuilder.create_element(
            self.contact, "exclude", body1=f"l{idx}", body2=f"r{idx}"
        )
    ...
    def _add_optional_components(
        self, body: etree.Element, idx: int, color: Color
    ) -> None:
    ...
    def _add_passive_ball(
        ...
        if self.mesh_dict["bottom"].endswith((".obj", ".stl")):
            self._add_geom_to_body(
                ...
                f"right{idx}",
                color_r,
                self.robot_cfg.bottom_hemi_mass,
                mesh="bottom",
                material="metallic",
                euler="180 0 60",
                friction="1.0 .0 .0",
    ...
    def add_stick(
        ...
        XMLBuilder.create_element(
            ...
            pos=pos,
            quat=quat,
            size=f"{radius} {length/2}",
            rgba=f"{color[0]} {color[1]} {color[2]} 1",
        )

    def add_independent_stick(
        self,
        parent_id,
        pos=None,
        quat=None,
        ...
        stick = XMLBuilder.create_element(
            parent,
            "body",
            name=f"passive{idx}",
            pos=vec2string(pos),
        ...
        if self.mesh_dict["stick"].endswith(".obj") or self.mesh_dict["stick"].endswith(
            ".stl"
        ):
            if broken != 0:
                assert (
                    "cut_stick" in self.mesh_dict
                ), "The cut_stick mesh should be provided"
            XMLBuilder.create_element(
                stick,
                "geom",
                name=f"stick{idx}",
                type="mesh",
                pos=vec2string(np.array([0, 0, 0]) + pos_offset),
                quat="1 0 0 0",
                mesh="stick" if broken == 0 else "cut_stick",
                rgba=f"{color[0]} {color[1]} {color[2]} 1",
                mass=f"{self.mass['stick']*(1-broken)}",
                friction="1.0 .0 .0",
                priority="2",
            )
            ...
            XMLBuilder.create_element(
                ...
                type="cylinder",
                pos=vec2string(
                    np.array([0, 0, length / 2 - length * (1 - broken) / 2])
                    + pos_offset
                ),
                quat="1 0 0 0",
                size=f"{radius} {length*(1-broken) /2 }",
                rgba=f"{color[0]} {color[1]} {color[2]} 1",
                mass=f"{self.mass['stick']*(1-broken)}",
                friction="1.0 .0 .0",
                priority="2",
            ...
            XMLBuilder.create_element(
                ...
                quat="1 0 0 0",
                size=f"{radius} {length/2 *(1-broken)}",
                rgba=f"{color[0]} {color[1]} {color[2]} 1",
                mass=f"{self.mass['stick']*(1-broken)}",
                friction="1.0 .0 .0",
                priority="2",
            ...
            raise ValueError(
                "The mesh should be either a .obj file or CYLINDER or CAPSULE"
            )
    ...
    def add_sock(
        ...
        XMLBuilder.create_element(
            self.leg_nodes[parent_id],
            "geom",
            type="cylinder",
            name=f"sock{self.sock_idx_counter}",
            pos=f"0 0 {-stick_length/2+length/2-thickness} ",
            quat="1 0 0 0",
            size=f"{radius} {length/2}",
            rgba=f"{color[0]} {color[1]} {color[2]} 1",
            mass="0",
    ...
    def add_simple_ball(
        ...
        """
        quat_r = construct_quaternion([1.0, 0, 0], np.pi)
        if not passive:
            self._add_one_module(
                parent_id, pos, quat, range, color, quat_r=quat_r, joint_axis=[0, 0, -1]
            )
        else:
            self._add_passive_ball(
                parent_id, pos, quat, range, color, quat_r=quat_r, joint_axis=[0, 0, -1]
            )
        return [len(self.leg_nodes) - 2, len(self.leg_nodes) - 1]
    ...
    def change_color_name(self, name: str, color: Color) -> None:
        ...
        for geom in self.tree.iter("geom"):
            if (
                "rgba" in geom.attrib
                and "name" in geom.attrib
                and name in geom.attrib["name"]
            ):
                geom.set("rgba", f"{color[0]} {color[1]} {color[2]} 1")
    ...
    def delete_body(self, body_name: str, keep_tag: str | None = None) -> None:
        """
                ...
                if keep_tag is None:
                    ...
                    for contact in self.root.findall("contact"):
                        for exclude in contact.findall("exclude"):
                            if (
                                exclude.attrib.get("body1") == body_name
                                or exclude.attrib.get("body2") == body_name
                            ):
    ...
    def delete_joint(self, joint_name: str) -> None:
        ...
        for actuators in self.tree.iter("actuator"):
            for actuator in actuators:
                if (
                    "joint" in actuator.attrib
                    and joint_name in actuator.attrib["joint"]
                ):
        ...
        if not filename.endswith(".xml"):
            if os.path.isdir(filename):
                ...
                filename = os.path.join(
                    METAMACHINE_ROOT_DIR, "assets", "robots", f"{filename}.xml"
                )

        try:
            self.tree.write(
                filename, pretty_print=True, xml_declaration=False, encoding="utf-8"
            )
            print(f"Saved the robot to {filename}")
            return filename
        except Exception as e:
            raise RobotBuilderError(f"Failed to save robot model: {str(e)}") from e
    ...
    def _build_torsor(self) -> None:
        ...
        XMLBuilder.create_element(
            torso,
            "camera",
            name=f"follow_camera{len(self.torsos)}",
            mode="track",
            pos="0 -2 1",
            xyaxes="1 0 0 0 1 2",
        )
    ...
    def _create_module_body(
        ...
        quat: str | Quaternion,
        color: Color,
        joint_params: dict[str, str] | None = None,
    ) -> etree.Element:
        ...
        body = XMLBuilder.create_element(parent, "body", name=name, pos=pos, quat=quat)
    ...
    def _add_module_sensors(
        self, l_body: etree.Element, r_body: etree.Element, idx: int
    ) -> None:
        ...
        for sensor_type, name in zip(sensor_types, sensor_names):
            XMLBuilder.create_element(
                self.sensors, sensor_type, name=f"{name}{idx}", site=f"imu_site{idx}"
            )
        ...
        XMLBuilder.create_element(
            self.sensors, "gyro", name=f"back_imu_gyro{idx}", site=f"back_imu_site{idx}"
        )
        XMLBuilder.create_element(
            self.sensors,
            "velocimeter",
    ...
    def _add_actuator(self, joint_name: str, **kwargs) -> None:
        ...
        XMLBuilder.create_element(
            self.actuators, "motor", ctrlrange="-12 12", joint=joint_name
        )

    def _validate_port_id(self, port_id: int | None) -> None:
        """Validate a port ID."""
    ...
    def _get_mesh_geom_params(
        self, mesh_type: str, color: Color, mass: float, **kwargs
    ) -> dict[str, str]:
...
if __name__ == "__main__":
    ...
    builder = RobotBuilder(
        mesh_dict=mesh_dict_fine, robot_cfg=robot_cfg_air, terrain="flat"
    )
    # builder.add_stairs(start_distance = 3)
    # builder.add_walls()
    builder.add_simple_ball()
    builder.add_independent_stick(
        0, pos=[0, 0, 0.1], quat=[1, 0, 0, 0], color=[1, 0, 0]
    )
