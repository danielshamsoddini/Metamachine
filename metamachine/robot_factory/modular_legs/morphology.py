...
class ModuleConnection:
    ...
    def to_sequence(self) -> list[int]:
        ...
        return [
            ...
            (
                self.parent_dock
                if isinstance(self.parent_dock, int)
                else self.parent_dock.value
            ),
            (
                self.child_dock
                if isinstance(self.child_dock, int)
                else self.child_dock.value
            ),
            self.orientation,
        ]
...
class RobotMorphology:
    ...
    def __init__(self, connections: Optional[list[ModuleConnection]] = None) -> None:
    ...
    def _update_module_id(self) -> None:
    ...
    def get_available_docks(self, module_id: int) -> list[DockPosition]:
        ...
        used_as_parents = {
            (
                conn.parent_dock
                if isinstance(conn.parent_dock, int)
        ...
        used_as_children = {
            (
                conn.child_dock
                if isinstance(conn.child_dock, int)
        ...
        return [dock for dock in DockPosition if dock.value not in used_docks]
    ...
    def validate_connection(self, connection: ModuleConnection) -> bool:
        ...
        parent_module_id = (
            ...
            if isinstance(connection.parent_module_id, int)
            else connection.parent_module_id.value
        )
        ...
        dock_value = (
            connection.parent_dock
            if isinstance(connection.parent_dock, int)
        ...
        if dock_value in used_docks:
            print(
                f"Dock {dock_value} already used in parent or child position for module {parent_module_id}"
            )
            return False

        # Check child dock is available
        child_dock_value = (
            connection.child_dock
            if isinstance(connection.child_dock, int)
            else connection.child_dock.value
        )
        if child_dock_value not in list(range(9)):
        ...
        orientation_value = (
            ...
            if isinstance(connection.orientation, int)
            else connection.orientation.value
        )
        if orientation_value not in available_rotations:
            print(
                f"Orientation {orientation_value} is not valid for parent dock {dock_value} and child dock {child_dock_value}"
            )
            return False
