...
def create_color_spectrum(
    colormap_name: str = "plasma", num_colors: int = 100
    ...
    try:
        ...
        raise ValueError(f"Invalid colormap name '{colormap_name}': {e}") from e
...
def euler_to_rotation_matrix(
    euler_angles: Union[list[float], np.ndarray],
) -> np.ndarray:
...
def random_point_at_distance(
    origin: Union[list[float], np.ndarray], distance: float
) -> np.ndarray:
...
def lighten_color(
    rgb: Union[list[float], tuple[float, float, float]], factor: float
) -> list[float]:
    ...
    if abs(alpha) < epsilon:
        ...
        jing_vec = np.array(
            [
                ...
                1,
                ((1 - cos(alpha)) ** 2 + sin(theta) * sin(alpha))
                / (cos(theta) * sin(alpha)),
            ]
        )
...
def torque_to_position_control(
    root: etree._Element, kp: float = 20, kd: float = 0.5
) -> etree._Element:
...
def compile_xml(
    xml_file: Union[str, os.PathLike],
    ...
    try:
        ...
        raise etree.XMLSyntaxError(f"Invalid XML in {xml_file}: {e}") from e
